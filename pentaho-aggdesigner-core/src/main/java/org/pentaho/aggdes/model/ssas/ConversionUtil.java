 ...
 * <p>
 * TODO: Support Composite Keys (Not supported in Mondrian) TODO: Investigate Attribute Datatype value TODO: Support All
 * Level enabled feature TODO: Support Shared Dimensions? TODO: Investigate Cube Internet Sales, Dimension Internet
 * Sales Order Details, for degenerate status TODO: Investigate unique names between attributes and hierarchies
 *
 * @author mlowery
 */
public class ConversionUtil {
  ...
  /**
   * This object is used to represent the ROLAP Star Schema Tables within Analysis Services
   */
  private static class Table {
    ...
    public Table( String logicalName, String dbName, String dbSchemaName, List<Key> uniqueKeys,
                  String queryDefinition ) {
      this.logicalName = logicalName;
      this.dbName = dbName;
  ...
  /**
   * This object is used to represent the ROLAP Star Schema Joins within Analysis Services
   */
  private static class Join {
    ...
    public String toString() {
      return "[Join: parent=" + parentTable.logicalName + ";pkey=" + parentKeyObject + "; child="
        + childTable.logicalName + "; ckey=" + childKeyObject + "]";
    }
  }

  /**
   * This is a utility function that locates the closest table to root table in the table names list.
   *
   ...
   * <p>
   * this method parses SSAS's DataSourceView and generates a set of star schemas and join fragments
   *
  ...
  private static List<Table> generateStarModels( Element ssasDataSourceView ) {
    ...
    for ( int i = 0; i < keyrefs.size(); i++ ) {
      ...
      // bind the two.  If child table already has a parent table, make a duplicate table and update both as future
      // parents
        ...
        if ( childTableModel.logicalName.equals( childTable ) ) {
          ...
          for ( int k = 0; k < starTables.size() && !found; k++ ) {
            ...
            if ( parentTableModel.logicalName.equals( parentTable )
              ...
              logger.debug(
                "Adding Join: " + parentTable + "(" + parentKeyObject + ") to " + childTable + "(" + childKeyObject + ")" );
    ...
    for ( int i = 0; i < relationships.size(); i++ ) {
      ...
      for ( int j = 0; j < origTables.size() && !found; j++ ) {
        ...
        if ( childTableModel.logicalName.equals( childTable ) ) {
          ...
          for ( int k = 0; k < starTables.size() && !found; k++ ) {
            Table parentTableModel = starTables.get( k );
            if ( parentTableModel.logicalName.equals( parentTable ) && !parentTableModel.logicalName.equals( childTableModel.logicalName ) ) {
              // skip parent / child hierarchies
              logger
                .debug( "Adding Join: " + parentTable + "(" + parentKey + ") to " + childTable + "(" + childKey + ")" );
    ...
    for ( int i = 0; i < allMeasureGroups.size(); i++ ) {
      ...
      if ( isVirtualMeasureGroup( ssasMeasureGroup ) ) {
        ...
        String dataSourceViewID =
          getXPathNodeText( ssasMeasureGroup, "../../assl:Source/assl:DataSourceViewID" ).toLowerCase();
        ...
        if ( allTables == null ) {
          Element ssasDataSourceView = (Element) ssasDatabase.selectSingleNode(
            "assl:DataSourceViews/assl:DataSourceView[translate(assl:ID, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', "
        ...
        String factTableName =
          getXPathNodeText( ssasMeasureGroup, "assl:Measures/assl:Measure/assl:Source/assl:Source/assl:TableID" );
  ...
  private static void populateDimensions(
    ...
    for ( int j = 0; j < measureDimensions.size(); j++ ) {
      ...
      Element keyAttribute =
        (Element) databaseDimension.selectSingleNode( "assl:Attributes/assl:Attribute[assl:Usage='Key']" );
      String keyAttributeID = getXPathNodeText( keyAttribute, "assl:ID" );

      String foreignKey = null;

      // first look in the dimension object within the measures
      List nodes = measureDimension.selectNodes( "assl:Attributes/assl:Attribute[assl:AttributeID='" + keyAttributeID
      ...
      Element keyTableObject = (Element) measureDimension.selectSingleNode(
        "assl:Attributes/assl:Attribute[assl:AttributeID='" + keyAttributeID
          + "']/assl:KeyColumns/assl:KeyColumn/assl:Source/assl:TableID" );
      if ( keyTableObject == null ) {
        keyTableObject =
          (Element) keyAttribute.selectSingleNode( "assl:KeyColumns/assl:KeyColumn/assl:Source/assl:TableID" );
        nodes = keyAttribute.selectNodes( "assl:KeyColumns/assl:KeyColumn/assl:Source/assl:TableID" );
        if ( nodes.size() > 1 ) {
          logger.warn( "(2) Key Column foreign key contains more than one column in dimension " + dimensionName
            + ", SKIPPING DIMENSION" );
          continue;
        }
      }
        ...
        for ( Table table : tables ) {
          if ( table == factTable ) {
            ...
            if ( keyColumnObject == null ) {
              keyColumnObject =
                (Element) keyAttribute.selectSingleNode( "assl:KeyColumns/assl:KeyColumn/assl:Source/assl:ColumnID" );
            }
          ...
          if ( foreignKeyObject != null ) {
            if ( foreignKeyObject.columns.size() > 1 ) {
              logger.warn(
                "FOREIGN KEY " + foreignKeyObject + " CONTAINS MORE THAN ONE COLUMN IN DIMENSION " + dimensionName );
            } else {
      ...
      // get hierarchies
      populateHierarchies( mondrianDimension, cubeDimension, databaseDimension, keyAttribute, factTable, allTables,
        foreignKey );
  ...
  private static void populateHierarchyRelation(
    ...
    if ( currentTable == null ) {
      ...
      if ( currentTable == null ) {
        throw new AggDesignerException(
          "Error: " + ssasHierarchyID + " star schema branch not found.  " + factTable.logicalName + "."
            + factForeignKey );
      }
      currentTable.getRoot().parentJoin =
        new Join( factTable, new Key( factForeignKey ), currentTable, new Key( factForeignKey ) );
      logger.debug(
        "IMPLICIT STAR JOIN FOR " + currentTable.logicalName + "." + factForeignKey + " to " + factTable.logicalName
          + "." + factForeignKey );
    }
  ...
  private static void populateAttributeHierarchies(
    ...
    for ( int i = 0; i < cubeAttributes.size(); i++ ) {
      ...
      if ( cubeAttribute.selectSingleNode( "assl:AttributeHierarchyEnabled" ) != null
        ...
        // skip if degenerate dimension
        if ( tables.size() != 1 || !tables.get( 0 ).equals( factTable.logicalName ) ) {
          populateHierarchyRelation( mondrianHierarchy, tables, ssasDatabaseDimension, factTable, allTables, attribID,
            factForeignKey );
        }
